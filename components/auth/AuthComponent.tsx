...
const AuthPageLayout = ({ children, title, instructions, className }: {children: React.ReactNode, title: string, instructions?: string,className?:string}) => {
  ...
  return (
     <div className={"grid w-full grid-cols-1 items-center md:grid-cols-[1.25fr_1fr] "+className}>
      <div className="hidden md:block border-e-1 rounded-xl max-h-[110vh]">
        <Image
          width={700}
          height={500}
          src={loginImg}
          alt="logo"
          className="h-[100vh] w-[853px] object-cover rounded-e-xl"
        />
      </div>
      <div className="mx-auto w-[90%] flex h-screen flex-1 flex-col items-start justify-center gap-2 rounded-3xl  px-4">
        <div className="mx-auto w-full max-w-md bg-backgroud">
          ...
          {instructions&&<p className="text-sub mb-4">{instructions}</p>}
         {children}
        </div>
      </div>
    </div>
