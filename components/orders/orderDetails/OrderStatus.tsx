  ...
  return (
    <div className="mb-8 flex items-center justify-between px-4 w-full"> 
      {orderStatus.map((step, index) => (
        <div key={`orderStatus ${index}`} className={`flex items-center  ${index < orderStatus.length - 1 ?"flex-1":'shrink-1'}`}>
            ...
            >
              ...
              >
                <path
                  ...
                  fill={step.status !=='waiting' ? "#5A6AE8" : "#979EC5"}
                />
              </svg>
            </div>
            <span
              className={`mt-2 text-xs font-medium ${
                step.status !=='waiting' ? "text-primary" : "text-sub" }`}
            >
              {step.key}
            </span>
          </div>
          {index < orderStatus.length - 1 && (
            <div
              className={`h-0.5 flex-1 self-start mt-[.8rem] ${
                orderStatus[index + 1].status !=='waiting' ? "bg-primary" : "bg-sub"
              }`}
            />
          )}
        </div>
